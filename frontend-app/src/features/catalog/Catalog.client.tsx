...
// ✅ کست ایمن و تمیز بدون استفاده از unknown
...
export default function CatalogClient() {
  ...
  // استخراج دسته‌بندی‌ها و تعداد آن‌ها
  const categories = useMemo(() => {
    const map = new Map<string, number>();
    MOCK_CARS.forEach((c) =>
      map.set(c.category, (map.get(c.category) || 0) + 1),
        ...
        <div className="mb-6">
          ...
          <div className="mt-4 text-xs text-smoke font-mono">
            {mounted
              ...
              : `Showing ${filtered.length} of ${MOCK_CARS.length}`}
          </div>
        </div>

        {/* ✅ گرید خودروها (مطمئن شو که CarGrid تایپ CatalogCar را قبول می‌کند) */}
        <CarGrid cars={filtered} />
